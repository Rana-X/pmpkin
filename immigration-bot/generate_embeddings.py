"""Generate embeddings for all cases using Voyage AI."""
...
if not MONGODB_URI or not VOYAGE_API_KEY:
    raise EnvironmentError(
        ...
        "Required: MONGODB_URI, VOYAGE_API_KEY"
    )

DATABASE_NAME = "rfe_tool"
DELAY_BETWEEN_CALLS = 0.5  # seconds
MODEL = "voyage-law-2"  # Optimized for legal documents, 1024 dimensions


def generate_embedding(text, voyage_client, max_retries=2):
    """Generate embedding for text using Voyage AI.

    Args:
        text: Input text to embed
        voyage_client: Voyage AI client instance
        max_retries: Number of retry attempts

    Returns:
        list: Embedding vector (1024 dimensions) or None if failed
    """
    for attempt in range(max_retries):
        try:
            # Truncate if too long (Voyage has token limits)
            # Roughly 1 token = 4 chars, voyage-law-2 has 16k token limit
            max_chars = 60000
            ...
            if attempt < max_retries - 1:
                print(f"    ⚠️  Voyage API error (attempt {attempt + 1}/{max_retries}): {e}")
                time.sleep(2)
                continue
            else:
...
def main():
    ...
    print("=" * 80)
    print("VOYAGE AI EMBEDDING GENERATION PIPELINE")
    print("=" * 80)
    ...
    print(f"Using model: {MODEL} (1024 dimensions)")
    print("\n" + "-" * 80)
    ...
    for idx, case in enumerate(tqdm(cases_to_process, desc="Generating embeddings"), 1):
        ...
        try:
            ...
            embedding = generate_embedding(full_text, voyage_client)
